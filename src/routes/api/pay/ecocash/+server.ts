...
import { PAY_ECOCASH_URL, PAY_LESOTHO_KEY } from '$env/static/private';
...
export const GET: RequestHandler = async () => {
	return text('Hello from /api/ecocash/pay');
};
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const response = await fetch(PAY_ECOCASH_URL, {
			method: 'POST',
			headers: headers,
			body: raw,
			redirect: 'follow'
		...
		if (!response.ok) {
			console.error('Response not ok in /api/pay/ecocash: ', response);
